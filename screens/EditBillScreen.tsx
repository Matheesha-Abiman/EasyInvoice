...
type EditBillScreenRouteProp = RouteProp<RootStackParamList, 'EditBill'>;

interface Props {
    navigation: EditBillScreenNavigationProp;
    route: EditBillScreenRouteProp;
}

const EditBillScreen: React.FC<Props> = ({ route, navigation }) => {
    ...
    const fetchBillData = async (): Promise<void> => {
        try {
            ...
            const itemsData: BillItemForm[] = itemsSnap.docs.map((doc) => ({
                id: doc.id,
            ...
            setOriginalItemIds(itemsSnap.docs.map((doc) => doc.id));
    ...
    const saveBill = async (): Promise<void> => {
        ...
        try {
            ...
            for (const itemId of originalItemIds) {
                try {
                    await deleteDoc(doc(db, 'bills', billId, 'items', itemId));
                } catch (e) { }
            }
    ...
    if (loading || saving) {
        return <LoadingSpinner message={saving ? 'Saving...' : 'Loading...'} />;
    }
        ...
        >
            ...
            >
                ...
                <View className="flex-row items-center mb-6">
                    ...
                    <Text className="text-2xl font-bold text-text">Edit Invoice</Text>
                </View>
                ...
                {/* Buttons */}
                <TouchableOpacity
                    className="bg-secondary rounded-2xl h-14 flex-row justify-center items-center mb-4 shadow-lg shadow-secondary/40"
                    onPress={saveBill}
                    activeOpacity={0.8}
                >
                ...
                <TouchableOpacity
                    className="bg-card border border-card-border rounded-2xl h-14 flex-row justify-center items-center mb-6"
                    onPress={() => navigation.goBack()}
                    activeOpacity={0.8}
                >
                    <Text className="text-text-light text-base font-semibold">Cancel</Text>
                </TouchableOpacity>
            </ScrollView>
        </KeyboardAvoidingView>
    );
